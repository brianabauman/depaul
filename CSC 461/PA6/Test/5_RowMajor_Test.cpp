...
PerformanceTimer tARow;
PerformanceTimer tBRow;

#define f_eq(A,B) (  fabs(A - B) < 0.001f)
#define V_EQ(A,B) ((f_eq(A.x,B.x) ) && ( f_eq(A.y, B.y)) && (f_eq(A.z,B.z)) && (f_eq(A.w,B.w) ))

#if RowMajor_Test_Enable
TEST(RowMajor, SIMD_Tests, RowMajor_Test_Enable)
{
	// ---------------------------------------------------
	// Row major tests
	// ---------------------------------------------------
	...
	Vect4D vOut = rowMajor_test();

	// --------------- RowMajor SIMD ----------------------------------------------

	printf("           : rowMajor SIMD\n");

	Vect_Row_SIMD vOut_simd = rowMajor_SIMD_test();

	Trace::out("       rowMajor Orig: %f s   :%f %f %f %f\n", tARow.TimeInSeconds(), vOut.x, vOut.y, vOut.z, vOut.w);
	Trace::out("                SIMD: %f s   :%f %f %f %f\n", tBRow.TimeInSeconds(), vOut_simd.x, vOut_simd.y, vOut_simd.z, vOut_simd.w);
	Trace::out("       rowMajor SIMD: %f s \n", tBRow.TimeInSeconds());

	if (tBRow.TimeInSeconds() < 0.3f)
	{
		Trace::out("\n");
	...
	{
		Trace::out("               Ratio: %f\n", tARow.TimeInSeconds() / tBRow.TimeInSeconds());
	}
