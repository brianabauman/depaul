...
#if Mem12_Allocate_3_100_bottom_up_free_Test_Enable
TEST( Allocate_3_100_bottom_up_free , MemoryShakeOut, Mem12_Allocate_3_100_bottom_up_free_Test_Enable)
{
	...
	// ---- Verify B: -----------------------------------------------------------
		...
		// Used / Free
		CHECK_EQUAL( PTR_FIX(h->pFreeHead), 0x8550 );
		CHECK_EQUAL( PTR_FIX(h->pUsedHead), 0x42c0 );
	...
	// ---- Heap Stats ------------------------------------------------------
		...
		CHECK_EQUAL( h->mStats.currUsedMem, 0xc790 - 0x4290 );
		
		CHECK_EQUAL( h->mStats.currNumFreeBlocks, 1 );
		CHECK_EQUAL( h->mStats.currFreeMem, 0x42a0 );
