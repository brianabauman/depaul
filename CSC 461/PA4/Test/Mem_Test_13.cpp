...
#if Mem13_Allocate_3_100_Top_Down_free_Test_Enable
TEST( Allocate_3_100_Top_Down_free , MemoryShakeOut, Mem13_Allocate_3_100_Top_Down_free_Test_Enable)
{
	...
	// ---- Heap Stats ------------------------------------------------------
		...
		CHECK_EQUAL( h->mStats.currUsedMem, 0x42a0);
		
		CHECK_EQUAL( h->mStats.currNumFreeBlocks, 1 );
		CHECK_EQUAL( h->mStats.currFreeMem, 0x8510 );
	...
	// ---- BLOCK ----------------------------------------------------
		...
		CHECK_EQUAL( PTR_FIX(blkStart), PTR_FIX(hdrEnd) );
		CHECK_EQUAL( PTR_FIX(blkEnd), 0x8550 );
		CHECK_EQUAL( free->mBlockSize, 0x8510 );
