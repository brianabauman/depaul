...
object fp3 {
  ...
  def allEqual (xs : List[Int]) : Boolean = {
    xs match {
      case Nil => true
      case _::ys if (ys.length == 0) => true
      case y::ys if (ys.length == 1 && y == ys.head) => true
      case y::ys if (ys.length > 1 && y == ys.head) => allEqual(ys)
      case _ => false
    }
  }
  ...
  def delete1 [X] (x:X, ys:List[X]) : List[X] = {
    ys match {
      case Nil => Nil
      case z::zs if (z == x) => delete1(x, zs)
      case z::zs => z :: delete1(x, zs)
    }
  }
  ...
  def delete2 [X] (x:X, ys:List[X]) : List[X] = {
    ys match {
      case Nil => Nil
      case zs => for (z <- zs; if z != x) yield z
    }
  }
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    xs match {
      case Nil => Nil
      case y::ys if (ys.length == 0) => y :: removeDupes1(ys)
      case y::ys if (y == ys.head) => removeDupes1(ys)
      case y::ys => y :: removeDupes1(ys)
    }
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    def removeDupes2Helper [X] (a:Int, xs:List[X]) : List[(Int, X)] = {
      xs match {
      	case Nil => Nil
      	case y::ys if (ys.length == 0) => (a, y) :: removeDupes2Helper(0, ys)
      	case y::ys if (y == ys.head) => removeDupes2Helper(a + 1, ys)
      	case y::ys if (y != ys.head) => (a, y) :: removeDupes2Helper(1, ys) 
      }
    }

    xs match {
      case Nil => Nil
      case ys => removeDupes2Helper(1, ys)
    }
  }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
  	def splitAtHelper [X] (tempList:List[X], n:Int, xs:List[X]) : (List[X], List[X]) = {
      xs match {
        case Nil => (tempList.reverse, Nil)
        case y::ys if (tempList.length < n) => splitAtHelper(y :: tempList, n, ys)
        case ys => (tempList.reverse, ys)
      }
    }

    splitAtHelper(Nil, n, xs)
  }
